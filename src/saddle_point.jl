...
function reset_solution_weighted_average!(
    ...
    solution_weighted_avg.avg_primal_solutions .=
        zeros(Float64, length(solution_weighted_avg.avg_primal_solutions))
    ...
    solution_weighted_avg.avg_primal_obj_product .= zeros(Float64, length(solution_weighted_avg.avg_primal_solutions))
    return
end
...
function run_restart_scheme(
    ...
    if !do_restart
        ...
        buffer_state.delta_primal .= current_primal_solution .- last_restart_info.primal_solution
        buffer_state.delta_dual .= current_dual_solution .- last_restart_info.dual_solution
        buffer_state.delta_dual_product .= dual_product .- last_restart_info.dual_product
        buffer_state.delta_primal_product .= primal_product .- last_restart_info.primal_product
        buffer_state.delta_primal_obj_product .= primal_obj_product .- last_restart_info.primal_obj_product
...
function generic_final_log(
    ...
    if verbosity >= 1
        print("Terminated after $iteration ,CG total iteration $CG_total_iteration iterations: ")
